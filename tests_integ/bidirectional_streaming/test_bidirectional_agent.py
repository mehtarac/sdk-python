...
async def test_bidirectional_agent(agent_with_calculator, audio_generator, provider_config):
    ...
    async with BidirectionalTestContext(agent_with_calculator, audio_generator) as ctx:
        ...
        logger.info("provider=<%s>, response=<%s> | response", provider_name, text_outputs_turn1[0][:100])
        ...
        logger.info("provider=<%s>, total_responses=<%d> | total responses", provider_name, len(text_outputs_turn2))
        ...
        logger.info("total_events=<%d> | total events", len(ctx.get_events()))
        logger.info("text_responses=<%d> | text responses", len(text_outputs_turn2))
        logger.info("audio_chunks=<%d>, bytes=<%d> | audio chunks", len(audio_outputs), total_audio_bytes)
        logger.info("=" * 60)
