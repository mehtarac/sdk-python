...
class BidirectionalTestContext:
    ...
    async def say(self, text: str):
        ...
        logger.debug("bytes=<%d>, text=<%s> | queued audio", len(audio_data), text[:50])
    ...
    async def wait_for_response(
        ...
        while time.monotonic() - start_time < timeout:
            ...
            if len(current_events) - initial_event_count >= min_events:
                ...
                if elapsed_since_event >= silence_threshold:
                    logger.debug(
                        "events=<%d>, silence_seconds=<%.1f> | response complete",
        ...
        logger.warning("timeout_seconds=<%s> | response timeout", timeout)
            ...
            while self.active:
                try:
                    ...
                    if input_item["type"] == "audio_chunk":
                        ...
                        logger.debug("data=<%s> | sent direct", data_repr)
    ...
    async def _event_collection_thread(self):
        ...
        try:
            async for event in self.agent.receive():
                ...
                logger.debug("keys=<%s> | event collected", list(event.keys()))
