...
class BidiOpenAIRealtimeModel(BidiModel):
    ...
    def _convert_openai_event(self, openai_event: dict[str, any]) -> list[BidiOutputEvent] | None:
        ...
        # Audio output
            ...
            return [BidiAudioStreamEvent(audio=openai_event["delta"], format="pcm", sample_rate=24000, channels=1)]
        ...
        # Assistant text output events - combine multiple similar events
            ...
            return [
                self._create_text_event(openai_event["delta"], role.lower() if isinstance(role, str) else "assistant")
            ]
